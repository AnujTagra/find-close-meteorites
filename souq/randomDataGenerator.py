...
def generateContainerIdScannableIdMap():
    cIdList = [ ''.join(random.sample(list(string.digits),6)) for _ in range(2)]
    cscanIdList = ['P-'+random.choice(list(string.digits))+'-'+''.join(random.sample(list(string.digits+string.ascii_uppercase),8)) for _ in range(2)]
...
if __name__ == '__main__':
    ...
    cIdScanIdMap = generateContainerIdScannableIdMap()
    for i in range(1000):
        asin=generateAsin()
